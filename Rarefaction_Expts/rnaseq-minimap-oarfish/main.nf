...
// Convert SAM to BAM, filter by quality, and sort BY NAME for oarfish
process SAM_TO_FILTERED_BAM {
    ...
    output:
    tuple val(sample_id), path("${sample_id}.filtered.namesorted.bam") into sorted_bam_ch
    
    script:
    """
    # Convert SAM to BAM and filter by mapping quality in one step
    samtools view -bS -q ${params.mapq} ${sam_file} > ${sample_id}.filtered.bam
    
    # Sort by READ NAME (not coordinate) for oarfish
    samtools sort -n -@ ${params.threads} ${sample_id}.filtered.bam -o ${sample_id}.filtered.namesorted.bam
    
    # Clean up intermediate file
    rm ${sample_id}.filtered.bam
    """
}
...
process OARFISH_ANALYSIS {
    ...
    input:
    tuple val(sample_id), path(bam_file) from sorted_bam_ch
    ...
    script:
    """
    oarfish -j ${params.threads} -a ${bam_file} -o ${sample_id} ${params.oarfish_params}
    """
}
