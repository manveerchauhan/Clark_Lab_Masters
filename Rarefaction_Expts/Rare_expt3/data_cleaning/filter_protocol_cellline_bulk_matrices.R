...
# Protocol-CellLine Bulk Matrix Filtering Script
# Filters transcripts within each individual count matrix file for ≥10 counts
...
load_and_filter_matrix <- function(file_path, min_counts = 10) {
  tryCatch({
...
process_protocol_cellline_group <- function(sample_group, min_counts = 10) {
...
process_sample_sheet <- function(sample_sheet_path, output_suffix = "") {
  ...
  for (protocol_cellline in names(protocol_groups)) {
    ...
    # Process this group
    result <- process_protocol_cellline_group(group_data, min_counts = 10)
...
main_filtering <- function() {
  cat("=== Protocol-CellLine Bulk Matrix Filtering ===\n")
  cat("Output directory:", OUTPUT_DIR, "\n")
  cat("Filtering criteria: ≥10 counts within each individual file\n\n")
  ...
  overall_summary <- data.frame(
    ...
    filtering_criteria = "≥10 counts per file",
    stringsAsFactors = FALSE
  )
...
# Run the filtering
cat("Starting protocol_cellline filtering...\n")
main_filtering()
cat("Done!\n") 